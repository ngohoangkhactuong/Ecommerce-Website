...
@WebServlet({ "/admin/categories/index", "/admin/categories/create", "/admin/categories/update",
...
public class CategoryServlet extends HttpServlet {
	...
	public void search(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String proname = request.getParameter("catename");
		...
		try {
			...
			request.setAttribute("endPage", endPage);
			request.setAttribute("total", count);
			request.setAttribute("listPagination", this.categoryDAO.searchCateByName(index , 5 , proname));
		} catch (Exception e) {
			e.printStackTrace();
		}
